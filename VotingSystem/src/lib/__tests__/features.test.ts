...
describe('fetchFeatures', () => {
  it('merges features with user votes', async () => {
    const featuresData = [
      { id: 'f1', title: 'A', description: 'desc', voter_id: 'v1', created_at: '2026-01-01', upvotes_count: 2, downvotes_count: 1, score: 1 },
    ];
    const votesData = [{ feature_id: 'f1', vote_value: 1 }];

    const mockSelect = jest.fn().mockResolvedValue({ data: featuresData, error: null });
    ...
    const result = await fetchFeatures('voter-1');
  ...
  it('defaults my_vote to 0 when user has no vote', async () => {
    const featuresData = [
      { id: 'f1', title: 'A', description: 'desc', voter_id: 'v1', created_at: '2026-01-01', upvotes_count: 0, downvotes_count: 0, score: 0 },
    ];
  ...
  it('handles null counts gracefully', async () => {
    const featuresData = [
      { id: 'f1', title: 'A', description: 'desc', voter_id: 'v1', created_at: '2026-01-01', upvotes_count: null, downvotes_count: null, score: null },
    ];
  ...
  it('throws when features query fails', async () => {
    ...
    await expect(fetchFeatures('voter-1')).rejects.toEqual({ message: 'DB error' });
  });
  ...
  it('throws when votes query fails', async () => {
    ...
    await expect(fetchFeatures('voter-1')).rejects.toEqual({ message: 'Vote error' });
  });
});
